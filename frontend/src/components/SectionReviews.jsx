...
const inputClass = 'w-full rounded-xl border border-ink/10 bg-surface px-3 py-2.5 text-sm text-ink outline-none transition focus:border-accent/45 focus:ring-4 focus:ring-accent/10';

function Estrellas({ valor, onChange }) {
  const [hover, setHover] = useState(0);
  return <div className="flex gap-1">{[1, 2, 3, 4, 5].map((number) => <button key={number} type="button" onClick={() => onChange?.(number)} onMouseEnter={() => onChange && setHover(number)} onMouseLeave={() => onChange && setHover(0)} className={`text-2xl transition-transform hover:scale-110 focus:outline-none ${number <= (hover || valor) ? 'text-amber-500' : 'text-gray-300'} ${onChange ? 'cursor-pointer' : 'cursor-default'}`} aria-label={`${number} estrella${number > 1 ? 's' : ''}`}>★</button>)}</div>;
}
...
function ReviewCard({ review }) {
  ...
  return <article className="flex flex-col gap-3 rounded-2xl border border-ink/10 bg-white p-6"><div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold text-white ${color}`}>{initial}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-ink">{review.nombre}</p>{review.ciudad && <p className="text-xs text-[#4B6355]">{review.ciudad}</p>}</div></div><time className="shrink-0 text-xs text-ink/40">{date}</time></div><Estrellas valor={review.estrellas} /><p className="text-sm leading-relaxed text-ink/75">&ldquo;{review.texto}&rdquo;</p></article>;
}

function ModalExito({ onClose }) {
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4" onMouseDown={onClose} role="dialog" aria-modal="true" aria-label="Reseña enviada"><div className="w-full max-w-sm rounded-2xl bg-white p-8 text-center shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="mb-4 text-5xl">✅</div><h3 className="mb-2 text-xl font-bold text-ink">¡Gracias por tu reseña!</h3><p className="mb-6 text-sm leading-relaxed text-[#4B6355]">Tu reseña ha sido enviada y está <strong>pendiente de revisión</strong>. La publicaremos cuando esté revisada.</p><button onClick={onClose} className="rounded-xl bg-accent px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-green-700">Entendido</button></div></div>;
}
...
export default function SectionReviews() {
  ...
  return <section className="w-full bg-[#F7FAF8] px-4 py-12 sm:py-16 lg:py-20"><div className="mx-auto max-w-[1120px]">
    <div className="mb-10 flex flex-col justify-between gap-6 sm:flex-row sm:items-end sm:gap-8"><div><p className="mb-2 text-xs font-bold uppercase tracking-widest text-accent">Opiniones</p><h2 className="font-display text-[clamp(2.2rem,5vw,3.2rem)] font-black leading-tight text-ink">Lo que dicen<br />nuestros usuarios</h2></div>{mediaEstrellas && <div className="flex min-w-[180px] shrink-0 items-center gap-4 rounded-2xl border border-ink/10 bg-white p-5"><div><p className="text-4xl font-black text-ink">{mediaEstrellas}</p><div className="mt-1 flex gap-0.5">{[1, 2, 3, 4, 5].map((number) => <span key={number} className={number <= Math.round(Number(mediaEstrellas)) ? 'text-base text-amber-500' : 'text-base text-gray-300'}>★</span>)}</div></div><div><p className="text-sm font-medium text-[#4B6355]">Valoración media</p><p className="text-xs text-ink/45">+{reviews.length} reseñas</p></div></div>}</div>

    {cargando && <div className="py-16 text-center text-[#4B6355]"><div className="mb-3 text-4xl animate-pulse">🌾</div><p>Cargando reseñas...</p></div>}
    {!cargando && reviews.length === 0 && <div className="mb-10 rounded-2xl border-2 border-dashed border-ink/10 bg-white p-10 text-center"><svg className="mx-auto mb-4 h-12 w-12" viewBox="0 0 32 32" fill="none" aria-hidden="true"><rect width="32" height="32" rx="9" fill="#16a34a" /><path d="M16 24 L16 10" stroke="white" strokeWidth="1.6" strokeLinecap="round" /><ellipse cx="16" cy="13" rx="3" ry="1.8" fill="white" opacity="0.9" transform="rotate(-30 16 13)" /><ellipse cx="16" cy="13" rx="3" ry="1.8" fill="white" opacity="0.9" transform="rotate(30 16 13)" /><ellipse cx="16" cy="17" rx="3" ry="1.8" fill="white" opacity="0.75" transform="rotate(-20 16 17)" /><ellipse cx="16" cy="17" rx="3" ry="1.8" fill="white" opacity="0.75" transform="rotate(20 16 17)" /><line x1="9" y1="9" x2="23" y2="23" stroke="white" strokeWidth="1.8" strokeLinecap="round" opacity="0.5" /></svg><p className="mb-2 text-lg font-semibold text-ink">Todavía no hay reseñas</p><p className="mb-6 text-sm text-[#4B6355]">Sé el primero en compartir tu experiencia con CeliApp.</p><button onClick={() => setMostrarFormulario(true)} className="rounded-xl bg-accent px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-green-700">¡Deja tu reseña!</button></div>}
