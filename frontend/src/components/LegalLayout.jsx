...
export default function LegalPage({ title, children }) {
  return <div className="min-h-screen bg-surface text-ink"><header className="border-b border-ink/10 bg-surface/95 backdrop-blur"><div className="mx-auto flex h-16 max-w-[1120px] items-center justify-between px-4 sm:px-6"><Logo /><NavLink to="/" className="rounded-full border border-ink/10 px-4 py-2 text-sm font-semibold text-[#4B6355] transition hover:border-accent/30 hover:bg-accent/5 hover:text-accent">Volver a CeliApp</NavLink></div></header><main className="px-4 py-12 sm:py-16"><div className="mx-auto max-w-3xl"><p className="mb-3 text-xs font-bold uppercase tracking-[0.1em] text-accent">Información legal</p><h1 className="mb-4 font-display text-4xl font-bold tracking-[-0.02em] text-ink sm:text-5xl">{title}</h1><p className="mb-9 text-sm text-[#4B6355]">Última actualización: septiembre de 2026</p><article className="rounded-3xl border border-ink/10 bg-white p-6 shadow-[0_4px_24px_rgba(13,31,20,0.06)] sm:p-9"><div className="space-y-7 text-[0.9375rem] leading-7 text-[#4B6355]">{children}</div></article></div></main><LegalFooter /></div>;
}
