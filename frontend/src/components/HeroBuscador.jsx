  ...
  return <section id="inicio" className="overflow-hidden bg-surface px-4 py-10 sm:py-12 md:py-14 lg:py-20">
    <div className="mx-auto max-w-[1120px]">
      ...
      <form className="reveal mb-4 max-w-[680px] [transition-delay:200ms]" onSubmit={onBuscar}>
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-1.5 rounded-lg border border-ink/15 bg-white p-2 sm:grid-cols-[auto_minmax(260px,1fr)_auto_auto] sm:p-1.5">
          <span className="flex shrink-0 items-center pl-3 text-ink/35"><svg className="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 0 14 0z" /></svg></span>
          ...
          <button type="submit" disabled={loading || !ean.trim()} className="col-span-3 flex min-h-11 w-full shrink-0 items-center justify-center gap-1.5 rounded-lg bg-accent px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-green-700 disabled:cursor-not-allowed disabled:bg-accent/30 sm:col-span-1 sm:min-w-[126px] sm:w-auto">{loading ? 'Analizando…' : 'Verificar'}</button>
        </div>
      </form>
