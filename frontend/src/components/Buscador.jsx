      ...
      <div className="w-full" style={{ fontFamily: "'Satoshi', system-ui, sans-serif" }}>
        ...
        <section className="relative overflow-hidden" style={{
          ...
          <div className="relative max-w-5xl mx-auto px-5 sm:px-8 pt-20 pb-32">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-10"
            ...
            <form onSubmit={buscarProducto} className="w-full max-w-2xl mb-6">
              <div className="flex items-center gap-1 p-1.5 rounded-2xl" style={{
                ...
                <div className="relative flex-1">
                  ...
                  <input type="text" placeholder="Código EAN · prueba: demo · noapto · dudoso"
                ...
                <button type="button" onClick={() => setScannerOpen(true)}
                  className="p-3.5 rounded-xl transition-all hover:bg-white/10"
                  style={{ color: 'rgba(240,253,244,0.4)' }} title="Escanear código de barras">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            ...
            <div className="flex flex-wrap gap-12">
              {[{ valor: '+50.000', label: 'productos analizados' }, { valor: '99%', label: 'de precisión' }, { valor: 'Gratis', label: 'sin suscripción' }].map(s => (
                <div key={s.label}>
              ...
              <div className="max-w-2xl rounded-3xl overflow-hidden"
                ...
                <div className="bg-white">
                  {resultado.producto?.imagen_url && (
                    <div className="w-full relative overflow-hidden" style={{ height: '280px', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                      <div style={{
                        position: 'absolute', inset: 0,
                        backgroundImage: `url(${resultado.producto.imagen_url})`,
                        backgroundSize: 'cover', backgroundPosition: 'center',
                        filter: 'blur(28px) brightness(0.9) saturate(0.6)',
                        transform: 'scale(1.15)', opacity: 0.4,
                      }} />
                      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(249,249,247,0.3) 0%, rgba(249,249,247,0.5) 100%)' }} />
                      <div className="absolute inset-0 flex items-center justify-center p-8">
                        <img src={resultado.producto.imagen_url} alt={resultado.producto?.nombre ?? 'Producto'}
                          style={{ maxHeight: '220px', maxWidth: '85%', width: 'auto', objectFit: 'contain', filter: 'drop-shadow(0 12px 32px rgba(0,0,0,0.20))', position: 'relative', zIndex: 1 }}
                  ...
                  <div className="px-7 py-7">
                    ...
                    <div className="rounded-2xl p-5 mb-6" style={{ background: '#f4f3f0' }}>
                    ...
                    {wrongCount === 0 ? (
                      <div className="flex items-center justify-between py-3 px-4 rounded-xl mb-5"
                        ...
                        <p className="text-xs text-gray-500">¿El resultado no corresponde al producto que tienes?</p>
                        <button onClick={handleWrongProduct}
                          className="text-xs font-bold ml-3 shrink-0 text-gray-400 hover:text-emerald-600 transition-colors">
                          No es mi producto
                        </button>
                      </div>
                    ) : (
                      <div className="rounded-2xl overflow-hidden mb-5"
                        style={{ border: '1px solid rgba(16,185,129,0.2)', background: 'linear-gradient(135deg, #f0fdf4, #ecfdf5)' }}>
                        <div className="px-5 pt-5 pb-4">
                          <div className="flex items-start gap-3 mb-4">
                            ...
                            <div>
                              <p className="font-black text-gray-900 text-sm mb-1">¿Quieres un análisis más preciso?</p>
                              <p className="text-xs text-gray-600 leading-relaxed">
                                Haz una foto a la etiqueta de ingredientes o al frente del envase.
                                Nuestra IA lo identificará visualmente y te dará un resultado más fiable.
                              </p>
                            </div>
                          </div>
          ...
          <div className="max-w-5xl mx-auto px-5 sm:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
              <div>
                ...
                <h2 className="text-4xl sm:text-5xl font-black text-gray-900 leading-tight mb-6">¿Tienes alguna<br />pregunta?</h2>
                <p className="text-gray-500 text-base leading-relaxed mb-10">Estamos aquí para ayudarte. Si tienes dudas sobre un producto, quieres reportar un error o simplemente quieres saber más, escríbenos.</p>
                <div className="space-y-4">
                  {[{ icon: '✉', label: 'Email', val: 'hola@celiapp.es' }, { icon: '📍', label: 'Ubicación', val: 'Madrid, España' }, { icon: '⚡', label: 'Respuesta en', val: 'menos de 24 horas' }].map(c => (
                    <div key={c.label} className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-base shrink-0" style={{ background: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>{c.icon}</div>
                      <div><p className="text-xs text-gray-400 font-medium">{c.label}</p><p className="text-sm font-semibold text-gray-800">{c.val}</p></div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[{ label: 'Nombre', type: 'text', ph: 'Tu nombre' }, { label: 'Email', type: 'email', ph: 'tu@email.com' }].map(f => (
                      <div key={f.label}>
                        ...
                        <input type={f.type} placeholder={f.ph} required className="w-full px-4 py-3 rounded-xl text-sm text-gray-900 placeholder-gray-400 font-medium focus:outline-none" style={{ background: '#f4f3f0', border: 'none' }} />
                      </div>
                    ))}
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-gray-400 block mb-1.5">Asunto</label>
                    <select className="w-full px-4 py-3 rounded-xl text-sm text-gray-900 font-medium focus:outline-none appearance-none" style={{ background: '#f4f3f0', border: 'none' }}>
                      <option>Duda sobre un producto</option><option>Reportar un error</option><option>Sugerencia de mejora</option><option>Colaboración</option><option>Otro</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-gray-400 block mb-1.5">Mensaje</label>
                    <textarea rows={4} placeholder="Cuéntanos en qué podemos ayudarte…" required className="w-full px-4 py-3 rounded-xl text-sm text-gray-900 placeholder-gray-400 font-medium focus:outline-none resize-none" style={{ background: '#f4f3f0', border: 'none' }} />
                  </div>
                  <button type="submit" className="w-full py-4 rounded-xl font-semibold text-sm text-white" style={{ background: '#10b981' }}>Enviar mensaje</button>
                </form>
              </div>
            </div>
          </div>
        ...
        <footer style={{ background: 'linear-gradient(135deg, #070d0a 0%, #0e1e15 100%)' }} className="py-16">
          <div className="max-w-5xl mx-auto px-5 sm:px-8">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pb-10 mb-10" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div className="flex items-center gap-3">
                <span className="w-9 h-9 rounded-xl flex items-center justify-center font-black text-white text-sm" style={{ background: '#10b981' }}>C</span>
                <span className="font-black text-white text-lg">CeliApp</span>
              </div>
              <p className="text-sm text-center" style={{ color: 'rgba(240,253,244,0.3)' }}>Tu asistente personal para una dieta sin gluten segura</p>
              <div className="flex gap-6 text-xs" style={{ color: 'rgba(240,253,244,0.25)' }}>
                <a href="#" className="hover:text-white transition-colors">Privacidad</a>
                <a href="#" className="hover:text-white transition-colors">Términos</a>
              </div>
            </div>
            <p className="text-xs text-center" style={{ color: 'rgba(240,253,244,0.2)' }}>© 2026 CeliApp · Hecho con ♥ para la comunidad celíaca</p>
          </div>
        </footer>
        ...
        {toastMsg && (
          <div className="fixed bottom-6 right-6 z-50">
            <div className="flex items-center gap-3 px-5 py-3.5 rounded-xl text-white text-sm font-medium"
              ...
              <span>{toastMsg}</span>
              <button onClick={() => setToastMsg('')} style={{ color: 'rgba(255,255,255,0.4)' }}>
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
