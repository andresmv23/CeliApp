...
export default function Buscador() {
  ...
  return <>{scannerOpen && <Scanner onScanSuccess={(code) => { setEan(code); setScannerOpen(false); buscarProducto(null, code); }} onClose={() => setScannerOpen(false)} />}{fotoOpen && <FotoAnalisis ean={ean} onResult={handleFotoResult} onClose={() => setFotoOpen(false)} />}<div className="font-sans text-ink"><HeroBuscador ean={ean} setEan={setEan} loading={loading} error={error} onBuscar={buscarProducto} onAbrirScanner={() => setScannerOpen(true)} /><SectionDivider />{resultado && !loading && cfg && <ResultadoProducto resultado={resultado} cfg={cfg} fuenteTexto={fuenteTexto} urlFuente={urlFuente} esFavorito={esFavorito} onToggleFavorito={toggleFavorito} onAnalizarFoto={() => setFotoOpen(true)} onNuevaBusqueda={() => { setResultado(null); setEan(''); setError(null); setBusquedaCompletada(false); }} resultRef={resultRef} />}</div><SectionDivider /><ComoFuncionaPreview /><SectionDivider /><SectionReviews /><TickerBand /><Contacto /><Footer />{toastMsg && <div className="fixed inset-x-3 bottom-3 z-50 sm:left-auto sm:right-4 sm:bottom-4 sm:w-auto"><div className="flex items-center gap-3 rounded-2xl bg-ink px-5 py-3.5 text-sm font-medium text-surface shadow-[0_8px_32px_rgba(13,31,20,0.22)]"><span className="flex-1">{toastMsg}</span><button onClick={() => setToastMsg('')} className="flex shrink-0 text-surface/40 transition hover:text-surface" aria-label="Cerrar notificación">×</button></div></div>}<style>{`@keyframes ticker { from { transform: translateX(0); } to { transform: translateX(-50%); } } .reveal { opacity: 0; transform: translateY(18px); transition: opacity 0.55s ease, transform 0.55s ease; } .reveal.visible { opacity: 1; transform: none; }`}</style></>;
}
