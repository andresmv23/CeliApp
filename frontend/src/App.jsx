...
function Navbar() {
  ...
  const navLinks = [
    ...
    { to: '/como-funciona', label: 'Cómo funciona', end: true },
  ...
  return <nav className={[
    ...
    scrolled ? 'border-ink/10 bg-surface/[0.97] shadow-[0_1px_12px_rgba(13,31,20,0.05)]' : 'border-transparent bg-surface/[0.82]',
  ].join(' ')}>
    <div className="mx-auto max-w-[1120px] px-4 sm:px-6">
      <div className="flex h-16 items-center justify-between gap-4">
        <NavLink to="/" aria-label="Inicio CeliApp" className="flex shrink-0 items-center gap-2.5 no-underline"><Logo /></NavLink>
        <div className="hidden items-center gap-8 md:flex">
          {navLinks.map(({ to, label, end }) => <NavLink key={to} to={to} end={end} className={navLinkClass}>{label}</NavLink>)}
        </div>
...
export default function App() {
  return <BrowserRouter><AuthProvider><MainLayout /></AuthProvider></BrowserRouter>;
}
