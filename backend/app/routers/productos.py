...
from app.servicios import (
    obtener_producto_por_ean,
    obtener_producto_por_ean_v2,
)
...
from app.ia_client import (
    ...
    buscar_producto_similar_por_ean_v2,
    verificar_gluten_web_v2,
)
...
def _enriquecer_producto_v2(producto: Producto) -> Producto:
    """
    ...
    if not ingredientes.strip():
        print(f"\n🧾 [ENRIQUECER V2] Buscando ingredientes: {marca} - {nombre}")
    ...
    if not imagen_url.strip():
        print(f"\n🖼️ [ENRIQUECER V2] Buscando imagen: {marca} - {nombre}")
...
@router.get("/producto/v2/{ean}", response_model=ProductoAnalizado)
@limiter.limit("10/minute")
def buscar_producto_inteligente_v2(
    request: Request,
    ean: str,
    current_user=Depends(get_optional_user),
) -> ProductoAnalizado:
    ...
    if conn:
        ...
        if producto_db:
            print(f"\n✅ [CACHE HIT V2] Producto {ean} encontrado en BD local.")
    ...
    # 2. Open Food Facts
    print(f"\n🔍 [CACHE MISS V2] Buscando {ean} en Open Food Facts...")
    resultado_off = obtener_producto_por_ean_v2(ean)
    ...
    if resultado_off.encontrado:
        ...
        producto = _enriquecer_producto_v2(producto)
        ...
        print(f"\n[IA V2] Verificando gluten del producto OFF {ean}...")
        analisis = verificar_gluten_web_v2(producto)
    ...
    print(f"\n[IA V2] OFF no encontró {ean}; buscando referencias web...")
    producto = buscar_producto_similar_por_ean_v2(ean)
